...
class ClustersList(ModifiersList):
    ...
    def _check_if_clusters_list_correct(self, clusters_to_check, layer=None):
        """
        Checks if all clusters are sorted by
        actual modifier index on layer and no duplicates are in
        the list.

        Returns fixed list.
        Returns False, if some of cluster does not
        exists in this ClustersList
        """

        if layer is None:
            modifiers_list = self.get_list()
        else:
            modifiers_list = layer.get_list()

        if len(clusters_to_check) == 0:
            return clusters_to_check

        clusters = copy.copy(clusters_to_check)
        result = []
    ...
    # =================================
    # Layer - specific reparse methods
    # =================================
    def _reparse_this_layer(self):
        """
        Parse this layer's clusters.
        """
        y = self._clusters_parser.parse_recursively(
                self.get_list())
        if y is False:
            return False
        self._modifiers_list = y

    def _reparse_this_layer_full(self):
        """
        Parse this layer's actual modifiers.
        """
        x = self.get_full_actual_modifiers_list()
        y = self._clusters_parser.parse_recursively(x)
        if y is False:
            return False
        self._modifiers_list = y
